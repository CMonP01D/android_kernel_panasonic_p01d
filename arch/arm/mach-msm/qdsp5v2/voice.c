		...
		if (v->dev_state == DEV_READY) {
            ...
            do {
    			...
    			if (v->voc_state == VOICE_ACQUIRE) {
    				...
    				msecs_to_jiffies(500)); /* [SIMT-lisensen-110901] add timeout, fix reseting phone BT cannot call up normally.*/
                    if (0 == rc){
        			    MM_ERR("Wait for VOICE_CHANGE time out\n");
                    }
                    if (rc < 0) {
        			    MM_ERR("Waitting VOICE_CHANGE is interrupted\n");
                    }
    			} else {
    			    mutex_unlock(&voice.voc_lock);
...
static int voice_cal_rx_volume(struct voice_data *v) {   
    int vol;
