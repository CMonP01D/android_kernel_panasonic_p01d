...
#include <linux/syscalls.h> //for save log . [simt-zhanghui-110817]
...
#define TEST_LOG				0 //for save log .[simt-zhanghui-110817]
#define bootconfirm				1 //for  irq ctrl .[simt-zhanghui-110817]
struct atmel_ts_data {
	...
	uint8_t finger_type;
	uint8_t finger_support;//finger num
	uint16_t finger_pressed;
	uint8_t face_suppression;
	uint8_t grip_suppression;
	uint64_t Slide_timestamp;
...
uint8_t NoChipFlag = 1; 
#endif

int atmel_ts_fake_resume(void);

#if TEST_LOG//for save log
int fd = 0;
static int temp = 0;
void sd_open(void)
{	
	fd = sys_open("/data/templog.txt",O_CREAT | O_WRONLY,0);
}
void sd_write(char* data)
{
	temp = temp + strlen(data) + 3;
	sys_write(fd, data, strlen(data) + 1);
}
void sd_seek(void)
{	
	sys_lseek(fd,temp,0);
...
static ssize_t tpmsg_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
...
static int tpmsg_open(struct inode *inode, struct file *file)
{
	printk(KERN_INFO "<atmel.c> %s\n",__FUNCTION__); 
	return 0;
}
...
static int tpmsg_close(struct inode *inode, struct file *file)
{
	printk(KERN_INFO "<atmel.c> %s\n",__FUNCTION__);	
	return 0;
}
static int tpmsg_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	printk("<atmel.c> tpmsg_ioctl() enter. \n");
	
	/* cmd mapping */
	switch(cmd)
	{
		case ATMELTP_SET_PARA:
...
static const struct file_operations tp_msg_fops = {
	.owner = THIS_MODULE,
	.open = tpmsg_open,
	.release = tpmsg_close,
	.read= tpmsg_read,
...
static struct miscdevice tp_msg_device = {
...
static void check_calibration(struct atmel_ts_data*ts)
{
	...
	if (data[T37_MODE] == T6_CFG_DIAG_CMD_TCH &&
		data[T37_PAGE] == T37_PAGE_NUM0) {
		x_limit = T28_CFG_MODE0_X + ts->config_setting.config_T28[T28_CFG_MODE];
		x_limit = x_limit << 1;
		if (x_limit <= 40) {
...
#endif
// [simt-zhanghui-110817]{ for save log start.
printk("tch_ch = %d, atch_ch = %d\n", tch_ch, atch_ch);
#if TEST_LOG	
{
	sd_open();
	sd_seek();
	...
	sprintf(data, "%d", tch_ch);
	sd_write("tch_ch = ");
	sd_write(data);
	memset(data, 0, 10);

	sd_seek();
	sprintf(data, "%d", atch_ch);
	sd_write("atch_ch = ");
	sd_write(data);
	memset(data, 0, 10);
...
//annie add start 08010
	if(atch_ch + tch_ch > 0)
	{
		i2c_atmel_write_byte_data(ts->client,
			get_object_address(ts, GEN_COMMANDPROCESSOR_T6) +
			T6_CFG_CALIBRATE, 0x55);
		printk("Re-calibration after check calibration\n");
	}
//annie add end
}
...
static void confirm_calibration(struct atmel_ts_data *ts)
{
	...
	{
		i2c_atmel_write_byte_data(ts->client,
		get_object_address(ts, GEN_ACQUISITIONCONFIG_T8) + T8_CFG_ATCHCALST, 0);
		i2c_atmel_write_byte_data(ts->client,
		get_object_address(ts, GEN_ACQUISITIONCONFIG_T8) +T8_CFG_ATCHCALSTHR, 1);
	}
	else if(ts->id->version >= 0x20)
	{
...
{
	if (ts->Recal_flag < RECALIB_DONE && ts->id->version == 0x16)
	...
	if (data[T9_MSG_STATUS] & T9_MSG_STATUS_RELEASE) 
	{
		...
		if (ts->finger_pressed & BIT(idx)) 
		{
			...
			//[simt-zhanghui-110817]}
			//if (ts->id->version >= 0x20 && ts->Recal_flag < RECALIB_DONE)
			if (ts->id->version >= 0x16 && ts->Recal_flag < RECALIB_DONE)
			{
				/* if there is no finger count and in need re-calibration flag is set, then check the time and position */
				if (ts->finger_count == 0 && !ts->Recal_flag)
				{
					if(idx == 0 && ts->finger_data[idx].y> 300 && (jiffies > ts->Slide_timestamp + HZ/5)
					&& ((abs(ts->finger_data[idx].x - ts->pre_data[idx].x) > 150)||(abs(ts->finger_data[idx].y - ts->pre_data[idx].y) > 150)))
					{
						confirm_calibration(ts);//want del for never confirm calibration. [simt-zhanghui-110817]
					}
				}
				if (ts->finger_count)
					i2c_atmel_write_byte_data(ts->client,
				...
				struct atmel_finger_data *fdata, uint8_t press, uint8_t last)
{
	if (!press){
		...
		/*else if(fdata->z !=0){
			VrpCallback((TouchSampleValidFlag|TouchSampleDownFlag), fdata->x, fdata->y);  
		}else{
...
static void atmel_ts_work_func(struct work_struct *work)
{
	...
	if (ts->id->version >= 0x15)
	{
		...
		/*if object is touch coordinate values*/
...
static int read_object_table(struct atmel_ts_data *ts)
{
	...
	for (i = 0; i < ts->id->num_declared_objects; i++) {
		...
		ts->object_table[i].i2c_address =	data[OBJ_TABLE_LSB] | data[OBJ_TABLE_MSB] << 8;
			 ...
			 const struct i2c_device_id *id)
{
	...
	//[simt-zhanghui-110817]}	
	{   
	    /*Read cfg_T5 structrue to get TP object report.*/
		for (loop_i = 0; loop_i < 10; loop_i++) {
			if (gpio_get_value(intr)) {
	...
	printk(KERN_INFO
		"\nAtmel TP info block: \nfamily_id = 0x%2.2X \nvariant_id = 0x%2.2X \nversion = 0x%2.2X \nbuild = 0x%2.2X \nmatrix_x_size = 0x%2.2X \nmatrix_y_size = 0x%2.2X \nnum_declared_objects = 0x%2.2X\n",
	...
	if (pdata) 
	{
		/*while (pdata->version > ts->id->version)//del. [simt-zhanghui-110817]
			pdata++;*/
		...
		{
		    /*write calibrate cmd and get checksum.*/
		...
		if (ts->id->version >= 0x20) 
		{
			...
			//ts->timestamp = jiffies + 60 * HZ;			//Need check .del .[simt-zhanghui-110817]
		}
		ts->config_setting.tchthr = pdata->tchthr;
		ts->config_setting.config_T7 = pdata->config_T7;
		ts->config_setting.config_T8 = pdata->config_T8;
		...
		/*---zhanghui_qst why write it two times*/
		if (ts->id->version >= 0x20 && ts->cal_tchthr) 
		{
			i2c_atmel_write_byte_data(ts->client,
				get_object_address(ts, TOUCH_MULTITOUCHSCREEN_T9) + T9_CFG_TCHTHR,
				ts->cal_tchthr);
	...
	//VrpInit(ts->input_dev, NULL);//del. [simt-zhanghui-110830]
...
//add. [simt-zhanghui-110817]{
	...
	//printk(KERN_ERR "<atmel.c>boot mode value. ( mode[0]=%x, mode[1]=%x).\n",sd_boot_mode[0],sd_boot_mode[1]);
	err = misc_register(&tp_msg_device);
	if (err) {
		printk(KERN_ERR "<atmel.c> tpmsg device register failed\n");
	}
//[simt-zhanghui-110817]}
...
static int atmel_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	printk("atmel tp version 0811,  add_sleep_poweron\n");//[simt-zhanghui-110817]
	disable_irq(client->irq);

	ret = cancel_work_sync(&ts->work);
	if (ret){
		enable_irq(client->irq);
		printk("Atmel TP atmel_ts_suspend cancel_work_sync error!\n");
	}
	ts->finger_pressed = 0;
	ts->finger_count = 0;
	//ts->first_pressed = 0;//del. [simt-zhanghui-110817]
...
static int atmel_ts_resume(struct i2c_client *client)
{
    ...
    //del. [simt-zhanghui-110817]
	/*if(ts->power){
		ts->power(ATMEL_POWER_ON);
	}
	msleep(5);
	if (ts->id->version >= 0x20 && ts->Recal_flag == RECALIB_NEED)*/
	{
		...
		i2c_atmel_write_byte_data(ts->client,
			get_object_address(ts, GEN_ACQUISITIONCONFIG_T8) + T8_CFG_ATCHCALSTHR,
			ts->cal_tchthr - 5);
		//ts->timestamp = jiffies;//del. [simt-zhanghui-110817]
	}
...
//[simt-zhanghui-110817]{
int atmel_ts_fake_resume() 
{	
	printk("<atmel.c> atmel_ts_fake_resume() enter. \n");

	if (NULL == myclient || 1 == NoChipFlag){
		printk("myclient == NULL\n");
	}
	else{
		struct atmel_ts_data *ts = i2c_get_clientdata(myclient);

		i2c_atmel_write_byte_data(myclient,
			get_object_address(ts, GEN_POWERCONFIG_T7) + T7_CFG_IDLEACQINT, 32);
		i2c_atmel_write_byte_data(myclient,
			get_object_address(ts, GEN_POWERCONFIG_T7) + T7_CFG_ACTVACQINT, 16);	

		i2c_atmel_write_byte_data(myclient,
			get_object_address(ts, GEN_COMMANDPROCESSOR_T6) +
			T6_CFG_CALIBRATE, 0x55);
		
		printk("<atmel.c> atmel_ts_fake_resume(): tp start work normal. \n");
		return 1;
	}
	return 0;
}

EXPORT_SYMBOL(atmel_ts_fake_resume);
