...
#endif /* defined (CONFIG_MACH_MAHIMAHI) */
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
			...
			//dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);//yu
...
#else
//[SIMT-yuhaipeng-20110829] don't do PM and DTIM setting to keep download speed when screen off.
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	DHD_TRACE(("%s: enter, value = %d in_suspend=%d\n",
		__FUNCTION__, value, dhd->in_suspend));
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	//yhp20110418, generate wlan0 instead of eth0
	/* Allocate device name */
	if (dev_alloc_name(net, "wlan%d") < 0) {
		DHD_ERROR(("%s: OOM - dev_alloc_name\n", __FUNCTION__));
		goto fail;
	}
