...
#if 0
#ifdef CONFIG_HAS_WAKELOCK //yuhaipeng 20110625
#include <linux/wakelock.h>
#endif
struct wake_lock softap_always_wake;
#endif
	...
	char *ext)
{
	...
	net_os_wake_unlock(dev);
#if 0
	printk("%s wake_unlock softap_always_wake\n",__func__);
	wake_unlock(&softap_always_wake);
	wake_lock_destroy(&softap_always_wake);
#endif
	return res;
}
	...
	char *extra)
{
	...
	net_os_wake_unlock(dev);
#if 0
	printk("%s:wake_lock softap_always_wake\n",__func__);
	wake_lock_init(&softap_always_wake, WAKE_LOCK_SUSPEND, "softap_always_wake");
	wake_lock(&softap_always_wake);
#endif
	return res;
}
